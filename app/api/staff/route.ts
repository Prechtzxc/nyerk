...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[POST /api/staff] STEP 3: calling createUser", { email })
    let userRecord
    try {
      ...
      return NextResponse.json(
        {
          ...
          step: "createUser",
          code: createError instanceof Error && "code" in createError ? (createError as any).code : undefined,
        },
        { status: 500 },
      )
    }
    console.log("[POST /api/staff] STEP 3 OK: user created", { uid: userRecord.uid })
